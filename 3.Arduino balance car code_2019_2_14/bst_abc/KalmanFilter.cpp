									...
									float R_angle,float C_0,float K1)
{
  ...
  //angle = 
  myfilter.filter(Angle, Gyro_x, dt, Q_angle, Q_gyro,R_angle,C_0);
  lkf_1d.filter(Angle);
angle = ekf_tg.filter(may/maz, Gyro_x);
  /*

  angle = ekf.filter(may/maz, Gyro_x, dt, Q_angle, Q_gyro,R_angle,C_0);
  */
  //��ת�Ƕ�Z�����
